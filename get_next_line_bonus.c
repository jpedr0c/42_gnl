/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jocardos <jocardos@student.42.rio>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/24 16:21:30 by jocardos          #+#    #+#             */
/*   Updated: 2022/05/25 13:43:58 by jocardos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

char	*read_aux_str(int fd, char *aux_str)
{
	int		reader_bytes;
	char	*buffer;

	buffer = malloc((BUFFER_SIZE + 1) * sizeof(char));
	if (!buffer)
		return (NULL);
	reader_bytes = 1;
	while (!ft_strchr(aux_str, '\n') && reader_bytes != 0)
	{
		reader_bytes = read(fd, buffer, BUFFER_SIZE);
		if (reader_bytes == -1)
		{
			free(buffer);
			return (NULL);
		}
		buffer[reader_bytes] = '\0';
		aux_str = ft_strjoin(aux_str, buffer);
	}
	free(buffer);
	return (aux_str);
}

char	*get_next_line(int fd)
{
	char		*line;
	static char	*aux_str[4096];

	if (BUFFER_SIZE < 1 || fd < 0)
		return (NULL);
	aux_str[fd] = read_aux_str(fd, aux_str[fd]);
	if (!aux_str[fd])
		return (NULL);
	line = return_line(aux_str[fd]);
	aux_str[fd] = new_aux_str(aux_str[fd]);
	return (line);
}
